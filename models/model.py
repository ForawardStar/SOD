...
class MetaNet(nn.Module):
    def __init__(self, param_dict):
        ...
        self.hidden = nn.Linear(128, 128*self.param_num)
...
class AdaptiveCrossAttention(nn.Module):
    def __init__(self, in_channels):
        ...
        self.proj_q = nn.Sequential(nn.Conv2d(hidden_dim, hidden_dim // 2, 4, stride = 4), MyConv2D(hidden_dim // 2, hidden_dim // 2, 3, stride=1), nn.Conv2d(hidden_dim // 2, hidden_dim, 4, stride = 4))
        self.proj_k = nn.Sequential(nn.Conv2d(hidden_dim, hidden_dim // 2, 4, stride = 4), MyConv2D(hidden_dim // 2, hidden_dim // 2, 3, stride=1), nn.Conv2d(hidden_dim // 2, hidden_dim, 4, stride = 4))
        self.proj_v = nn.Sequential(nn.Conv2d(hidden_dim, hidden_dim // 2, 4, stride = 4), MyConv2D(hidden_dim // 2, hidden_dim // 2, 3, stride=1), nn.Conv2d(hidden_dim // 2, hidden_dim, 4, stride = 4))
